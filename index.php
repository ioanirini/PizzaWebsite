...
<div class="w3-container w3-black w3-padding-64 w3-xxlarge" id="menu">
  ...
  <a href="javascript:void(0)" onclick="openMenu(event, 'Pites');">
        <div class="w3-col s4 tablink w3-padding-large w3-hover-red">Pites</div>
      </a>
    </div>
...
<?php
$sql = "SELECT id,title,price,description FROM food WHERE type='pizza'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
  // output data of each row
  while($row = mysqli_fetch_assoc($result)) {
    echo "<h1><b>" .$row["title"]."</b> <span class='w3-right w3-tag w3-dark-grey w3-round'>".$row["price"]."</span></h1>";
	echo "<p class='w3-text-grey'>".$row["description"]. "</p> <hr>";
  }
} else {
  echo "0 results";
}
...
<?php
$sql = "SELECT id,title,price,description FROM food WHERE type='pasta'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
  // output data of each row
  while($row = mysqli_fetch_assoc($result)) {
    echo "<h1><b>" .$row["title"]."</b> <span class='w3-right w3-tag w3-dark-grey w3-round'>".
	$row["price"]."</span></h1>";
...
?>
  ...
  <div id="Pites" class="w3-container menu w3-padding-32 w3-white">
      <h1><b>Today's Soup</b> <span class="w3-tag w3-grey w3-round">Seasonal</span><span class="w3-right w3-tag w3-dark-grey w3-round">$5.50</span></h1>
      <p class="w3-text-grey">Ask the waiter</p>
      <hr>
